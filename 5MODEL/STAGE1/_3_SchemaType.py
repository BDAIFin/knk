...
def build_transaction_schema(
    ...
    # 1) online flag 생성 + online만 필터
    trans["is_online"] = (trans["use_chip"] == "Online Transaction").astype(cfg.binary_dtype)
    trans = trans[trans["is_online"] == 1].copy()
    ...
    # 3) amount 정리
    trans["amount"] = (
        trans["amount"].astype(str).str.replace("$", "", regex=False).astype(cfg.amount_dtype)
    )
    ...
    trans["weekday"] = trans["date"].dt.weekday.astype(cfg.month_day_hour_dtype)
    ...
    # 7) use_chip drop (원하면 유지해도 됨)
    trans = trans.drop(columns=["use_chip"])

    # 8) mcc cleanup
    trans["mcc"] = pd.to_numeric(trans["mcc"], errors="coerce").astype(cfg.mcc_nullable_int_dtype)
