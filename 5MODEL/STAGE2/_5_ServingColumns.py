...
# ===========================
# Final Drop Columns (MERGED)
# ===========================
# - 아래 컬럼들은 최종 feature output에서 제외(drop)됨
# - 생성 로직은 삭제하지 않고, "최종 선택 단계"에서 drop 처리
FINAL_DROP_COLUMNS = [
    # history count 중복
    "client_weekday_prior_count",

    # 식별자 (모델 입력에서는 제외하되, id는 output 키로 유지 가능)
    "id",
...
# ===========================
# 주의:
# - 아래 리스트는 "원본(101)" 그대로 유지
# - FINAL_DROP_COLUMNS에 포함된 항목은 "최종 output에서 제외"되도록 후처리에서 drop됨
STAGE2_FEATURES_101_RAW = [
    # 1) Refund / Amount
    "refund_high_amount",
    "log_abs_amount",
    ...
    # 16) Minimal customer/card context duplicates
    "is_credit",
    "has_chip",
...
# 최종 학습/서빙 feature list (drop 적용)
# - "드랍해야 할 코드는 지우지 말고" => 위 RAW 리스트는 유지
# - 실제 모델 입력 컬럼만 여기서 필터링
STAGE2_FEATURES_101 = [c for c in STAGE2_FEATURES_101_RAW if c not in set(FINAL_DROP_COLUMNS)]
...
def build_features_101(
    ...
    # 주의: id는 output 키로 유지 가능(모델 입력에서는 제외 가능)
    ...
    # ---- seconds_since_prev_tx (client 기준)
    df = ensure_sorted(df, ["client_id", "date", "_row_id"])
    ...
    # ---- final: restore row order
    df = df.sort_values("_row_id", kind="mergesort").reset_index(drop=True)

    # -----------------------------
    # Final feature selection
    # -----------------------------
    # 원래: keep = ["id"] + STAGE2_FEATURES_101 + ([LABEL] if LABEL in df.columns else [])
    # 변경:
    # - id는 output key로 항상 유지
    # - STAGE2_FEATURES_101는 이미 FINAL_DROP_COLUMNS 적용된 버전
    ...
    # -----------------------------
    # Drop columns (safety net)
    # -----------------------------
    # - "드랍해야 할 코드 삭제 X" => 생성은 그대로, output에서만 drop
    # - 단, id는 항상 남김(키로 필요)
    drop_in_output = [c for c in FINAL_DROP_COLUMNS if c in df_out.columns and c != "id"]
    if drop_in_output:
        df_out = df_out.drop(columns=drop_in_output)
